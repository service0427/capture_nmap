...
def generate_routes(place_id, count=1):
    name, store_lng, store_lat = get_place_info(place_id)
    print(f"[*] Reverting to Original PBF Mode for: {name}")
    ...
    while success_count < count and attempt < 20:
        ...
        headers = {
            ...
            "x-adid": "f43946e9-bcd3-4cee-a5fb-dea35a25c416",
            "x-agreement": "0",
        ...
        try:
            ...
            if resp.status_code == 200:
                ...
                if route_pts:
                    filename = f"Target_Route_01.json" # 파일명 고정
                    with open(f"{LIB_DIR}/{filename}", "w") as f:
                        json.dump(route_pts, f)
                    print(f" [✓] Original Success! Created: {filename}")
                    success_count += 1
            else:
    ...
    print(f"\n[DONE] Generated {success_count} original routes.")

if __name__ == "__main__":
    generate_routes("1889658893", 1)
