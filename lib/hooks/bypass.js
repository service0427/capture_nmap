...
Java.perform(function() {
    ...
    if (m && m !== "none") {
        try {
            ...
            // 해상도 변조 (DisplayMetrics)
            if (scr_width && scr_height && scr_density) {
                try {
                    var Resources = Java.use("android.content.res.Resources");
                    var origGetDM = Resources.getDisplayMetrics;
                    var w = parseInt(scr_width);
                    var h = parseInt(scr_height);
                    var d = parseFloat(scr_density);
                    var dpi = Math.round(d * 160);
                    
                    // DisplayMetrics 패치 공통 함수
                    var patchDM = function(dm) {
                        dm.widthPixels.value = w;
                        dm.heightPixels.value = h;
                        dm.density.value = d;
                        dm.densityDpi.value = dpi;
                        dm.xdpi.value = dpi;
                        dm.ydpi.value = dpi;
                        dm.scaledDensity.value = d;
                    };
                    origGetDM.implementation = function() {
                        var dm = origGetDM.call(this);
                        patchDM(dm);
                        return dm;
                    };
                    
                    // Display.getRealSize / getSize → Point(x,y) 변조
                    var Display = Java.use("android.view.Display");
                    var sizeHooks = ["getRealSize", "getSize"];
                    sizeHooks.forEach(function(method) {
                        try {
                            Display[method].overload("android.graphics.Point").implementation = function(pt) {
                                this[method](pt);
                                pt.x.value = w;
                                pt.y.value = h;
                            };
                    ...
                    // Display.getRealMetrics / getMetrics → DisplayMetrics 변조
                    var metricsHooks = ["getRealMetrics", "getMetrics"];
                    metricsHooks.forEach(function(method) {
                        try {
                            Display[method].overload("android.util.DisplayMetrics").implementation = function(dm) {
                                this[method](dm);
                                patchDM(dm);
                            };
                        } catch(e_m) {}
                    ...
                    results.DISPLAY = { to: w + "x" + h + "@" + d, ok: true };
                } catch(e_dm) {
                    results.DISPLAY = { ok: false, err: e_dm.toString() };
                }
            } else {
    ...
    if (adid) {
        ...
        try {
            ...
            // toString() 시점에 캐싱된 adid도 강제 교체 (중첩 포함)
            ...
            origToString.implementation = function() {
                try {
                    ...
                    // 중첩 JSONObject 내 adid 체크 (usr, body 등)
                    var keys = this.keys();
                    while (keys.hasNext()) {
                        var k = keys.next();
                        try {
