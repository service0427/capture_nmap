import requests
import hmac
import hashlib
import base64
import time
import urllib.parse
import random
import math
import json
import os
import sys

class NaverCrypto:
    @staticmethod
    def generate_drive_hmac(url, timestamp_ms=None):
        KEY_DRIVE_ENCRYPT = b"fvOvQAZ5fvDMvQqiQ6KTZYpPkGhr0oQp653TDil12acO5wnhvIQhl5veOvjoku0H"
        msgpad = str(timestamp_ms if timestamp_ms else int(time.time() * 1000))
        payload = (url[:255] + msgpad).encode('utf-8')
        h = hmac.new(KEY_DRIVE_ENCRYPT, payload, hashlib.sha1).digest()
        md = "v0:" + base64.b64encode(h).decode('utf-8')
        return msgpad, md

class RouteDecoder:
    @staticmethod
    def decode_json_path(coords_array):
        if not coords_array or len(coords_array) < 2: return []
        pts = []
        curr_x, curr_y = coords_array[0], coords_array[1]
        pts.append([float(curr_y) / 10000000.0, float(curr_x) / 10000000.0])
        for i in range(2, len(coords_array), 2):
            if i + 1 < len(coords_array):
                curr_x += coords_array[i]; curr_y += coords_array[i+1]
                pts.append([float(curr_y) / 10000000.0, float(curr_x) / 10000000.0])
        return pts

    @staticmethod
    def decode_pbf_path(resp_content: bytes):
        import gzip
        try:
            if resp_content[:2] == b'\x1f\x8b': resp_content = gzip.decompress(resp_content)
        except Exception: pass
        def decode_zigzag(n): return (n >> 1) ^ (-(n & 1))
        for i in range(len(resp_content) - 4):
            if resp_content[i] == 0x0a:
                try:
                    idx = i + 1; length = 0; shift = 0
                    while True:
                        b = resp_content[idx]; idx += 1
                        length |= (b & 0x7f) << shift
                        shift += 7
                        if not (b & 0x80): break
                    if 10 < length < 1000000 and idx + length <= len(resp_content):
                        arr = resp_content[idx:idx+length]; idx2 = 0; coords = []
                        while idx2 < len(arr):
                            val = 0; s2 = 0
                            while True:
                                b = arr[idx2]; idx2 += 1
                                val |= (b & 0x7f) << s2
                                s2 += 7
                                if not (b & 0x80): break
                            coords.append(decode_zigzag(val))
                        if len(coords) >= 2:
                            lng, lat = coords[0], coords[1]
                            if 1200000000 < lng < 1350000000 and 300000000 < lat < 450000000:
                                return RouteDecoder.decode_json_path(coords)
                except Exception: pass
        return []

def get_route_config():
    config_path = "lib/data/route_config.json"
    if os.path.exists(config_path):
        with open(config_path, 'r', encoding='utf-8') as f:
            return json.load(f)
    return {
        "default_id": "1889658893",
        "manual_start": {"enabled": False},
        "random_radius_km": 10.0,
        "saved_destinations": [
            {"name": "달빛잔기지떡", "id": "1889658893", "lng": 126.6721551, "lat": 37.6428285}
        ]
    }

def get_random_point(lat, lng, radius_km):
    radius_deg = radius_km / 111.0
    u, v = random.random(), random.random()
    w = radius_deg * math.sqrt(u); t = 2 * math.pi * v
    return lat + w * math.sin(t), lng + (w * math.cos(t)) / math.cos(math.radians(lat))

def _add_return_segment(route_pts):
    if len(route_pts) < 10: return route_pts
    n_return = 1
    backtrack_source = route_pts[-(n_return + 1):-1]
    backtrack_source.reverse()
    return_pts = []
    for pt in backtrack_source:
        jitter_lat = random.uniform(-0.00008, 0.00008)
        jitter_lng = random.uniform(-0.00008, 0.00008)
        return_pts.append([pt[0] + jitter_lat, pt[1] + jitter_lng])
    return route_pts + return_pts

def generate_routes(target_id=None, count=1):
    config = get_route_config()
    saved = config.get("saved_destinations", [])
    
    # [LOGIC] Match Target Destination
    selected_dest = None
    
    # 1. Default fallback if no target_id
    if not target_id:
        target_id = config.get("default_id")
    
    # 2. Try ID Match
    for d in saved:
        if d["id"] == str(target_id):
            selected_dest = d
            break
            
    # 3. Try Index Match (Serial Number) if no ID match and target_id is numeric
    if not selected_dest and str(target_id).isdigit():
        idx = int(target_id)
        if 0 <= idx < len(saved):
            selected_dest = saved[idx]
    
    # 4. Final Fallback to Default ID if still not found
    if not selected_dest:
        default_id = config.get("default_id")
        for d in saved:
            if d["id"] == str(default_id):
                selected_dest = d
                break

    if not selected_dest:
        print(" [!] Critical Error: No destination found. Check route_config.json")
        return

    print(f"[*] Starting Route Generation:")
    print(f"    > Destination: {selected_dest['name']} ({selected_dest['id']})")
    
    LIB_DIR = "route_library"
    os.makedirs(LIB_DIR, exist_ok=True)
    
    success_count = 0
    attempt = 0
    while success_count < count and attempt < 20:
        attempt += 1
        
        if config.get("manual_start", {}).get("enabled", False):
            start_lat = config["manual_start"]["lat"]
            start_lng = config["manual_start"]["lng"]
            print(f"    > Attempt {attempt}: Using Manual Start ({start_lat}, {start_lng})")
        else:
            radius = config.get("random_radius_km", 10.0)
            start_lat, start_lng = get_random_point(selected_dest["lat"], selected_dest["lng"], radius)
            print(f"    > Attempt {attempt}: Using Random Start within {radius}km")
        
        base_url = "https://drive.io.naver.com/v3/global/driving"
        params = {
            "mainoption": "traoptimal:tracomfort:traoptimal,avoidtoll:traoptdist:traoptimal,multiroute",
            "rptype": "4",
            "routesession": "1",
            "start": f"{start_lng},{start_lat}",
            "goal": f"{selected_dest['lng']},{selected_dest['lat']}",
            "startexcludefilter": "24",
            "goalexcludefilter": "0",
            "autochange": "1",
            "laneall": "1",
            "etatype": "0",
            "uuid": "85441208bbd688a8ca5a1bc0d2d230d",
            "caller": "mapmobileapps_Android_35_app6.4.0.7_fw2.12.6",
            "lang": "ko",
            "mileage": "11.9",
            "fueltype": "1",
            "cartype": "1",
            "vehicletype": "car",
            "crs": "EPSG:4326",
            "output": "pbf",
            "respversion": "9"
        }
        
        full_url = f"{base_url}?{urllib.parse.urlencode(params)}"
        msgpad, md = NaverCrypto.generate_drive_hmac(full_url)
        
        headers = {
            "device-id": "85441208bbd688a8ca5a1bc0d2d230d",
            "x-consumer-id": "mapmobileapps.mapmobileapps",
            "x-hmac-msgpad": msgpad,
            "x-hmac-md": md,
            "user-agent": "ktor-client",
            "accept": "*/*",
            "accept-encoding": "gzip",
            "referer": "client://NaverMap"
        }
        
        try:
            resp = requests.get(full_url, headers=headers, timeout=10)
            if resp.status_code == 200:
                route_pts = RouteDecoder.decode_pbf_path(resp.content)
                if route_pts:
                    route_pts = _add_return_segment(route_pts)
                    filename = f"Target_Route_01.json"
                    with open(f"{LIB_DIR}/{filename}", "w") as f:
                        json.dump(route_pts, f)
                    print(f" [✓] Success! Created: {filename}")
                    success_count += 1
            else:
                print(f" [!] API Error {resp.status_code}")
        except Exception as e:
            print(f" [!] Error: {e}")
        time.sleep(0.5)

    print(f"\n[DONE] Generated {success_count} routes.")

if __name__ == "__main__":
    # Command line usage: python3 smart_route_gen.py [ID or Index]
    arg_id = sys.argv[1] if len(sys.argv) > 1 else None
    generate_routes(target_id=arg_id, count=1)
